...
def scanline_convert(polygons, i, screen, zbuffer):
    tri = [polygons[i + j] for j in range(3)]
    tri.sort(key=lambda x: x[1])
    xb, xm, xt = [tri[j][0] for j in range(3)]
    yb, ym, yt = [tri[j][1] for j in range(3)]
    zb, zm, zt = [tri[j][2] for j in range(3)]
    ...
    flag=True
    while y <= yt:
        draw_line(int(x0), int(y), z0, int(x1), int(y), z1, screen, zbuffer, [(13*i)%255,(4*i+20)%255,(73*i+40)%255])
        x0 += dx0
        x1 += dx1
        z0 += dx0
        z1 += dz1
